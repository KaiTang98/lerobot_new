...
class OculusReader:
    def __init__(self,
        ...
        self.haptic_reset_interval = 1.0  # seconds
    ...
    def force_stop_app(self):
        """Force-stop the Quest app via ADB to fully close it on disconnect."""
        try:
            # Use Activity Manager force-stop to kill the package
            self.device.shell(f'am force-stop {self.APK_name}')
        except Exception:
            # As a fallback, try via host adb if device.shell path is problematic
            try:
                os.system(f'adb shell am force-stop {self.APK_name}')
            except Exception:
                pass
